...
export default function HouseNumberPicker({ selectedHouse, onSelectHouse }: HouseNumberPickerProps) {
  ...
  const renderRangeButtons = () => {
    ...
    for (let i = 1; i <= 75; i += 10) {
      ...
      const isSelectedHouseInRange = selectedHouse !== null && selectedHouse >= i && selectedHouse <= end
    ...
    for (let i = start; i <= end; i++) {
      buttons.push(
        <button
          ...
          className={`h-12 w-12 rounded-md text-lg font-medium transition-colors ${
            selectedHouse === i ? "bg-emerald-600 text-white" : "bg-zinc-700 hover:bg-zinc-600 text-zinc-100"
          }`}
        >
          {i}
        </button>,
      ...
      {selectedRange ? (
        <div>
          <div className="flex items-center mb-4">
            <button onClick={handleBackToRanges} className="mr-2 p-1 rounded-md hover:bg-zinc-700 transition-colors">
          ...
          <div className="grid grid-cols-5 gap-2 mb-4">{renderHouseNumberButtons()}</div>
        </div>
      ) : (
        <div>
          <h3 className="text-lg font-medium mb-4">Select House Number Range</h3>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-4">{renderRangeButtons()}</div>
        </div>
      )}
